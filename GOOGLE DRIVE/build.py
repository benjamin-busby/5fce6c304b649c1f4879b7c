...
# --- ensure same-folder imports work regardless of working directory ---
HERE = Path(__file__).resolve().parent
if str(HERE) not in sys.path:
    sys.path.insert(0, str(HERE))

from Agent1 import run_from_gui  # returns IngestResult
...
def append_line_to_label(lbl: tk.Label, line: str):
    """Thread-safe append of a line to a Label 'work log'."""
    def _do():
...
def add_left_row(r, btn_text, btn_bg, btn_active_bg, command, worklog_text=""):
    btn = tk.Button(rows, text=btn_text,
                    font=BUTTON_FONT,
                    fg="white", bg=btn_bg,
...
# ====== Analyse CVs action (unchanged demo) ======
def analyse_cvs():
    ...
    try:
        ...
        patches = hist_out[2]
        for p in patches: p.set_facecolor(BAR
